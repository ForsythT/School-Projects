...
void decision_choices(int length, int, char *array, char *alphabet, char morse[][6], char *letter);
int decision();
...
int main(){
	...
	int answer = decision(), length;
		decision_choices(length, answer, array, alphabet, morse, letter);	
	}
...
void decision_choices(int length, int answer, char *array, char *alphabet, char morse[][6], char *letter){
	switch(answer){
		case 0:
			exit(0);
		case 1:
			length = get_array(array);
