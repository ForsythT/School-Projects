...
void pq_insert(struct pq* pq, void* item, int priority) {
   ...
   */
	...
	if(dynarray_size(pq->heap) > 1){
		...
		struct pq_elem* parent = dynarray_get(pq->heap, parent_idx); 
		while(new_elem->priority < parent->priority){
			dynarray_set(pq->heap, i, parent);
			dynarray_set(pq->heap, parent_idx, new_elem);
			i = parent_idx;
			parent_idx = (i - 1) / 2;
...
void* pq_remove_first(struct pq* pq) {
   ...
   * value there in first_elem.
   */
	last_elem = dynarray_get(pq->heap, -1);
   ...
   */
	...
	while(cont == 1){
		cont = 0;
		if(dynarray_size(pq->heap) > (2*i)+2){
			struct pq_elem* right_child = dynarray_get(pq->heap, (2*i)+2);
			struct pq_elem* left_child = dynarray_get(pq->heap, (2*i)+1);
			if(right_child->priority > left_child->priority){
				if(left_child->priority < last_elem->priority){
					dynarray_set(pq->heap, i, left_child);
					dynarray_set(pq->heap, (2*i)+1, last_elem);
					i = (2*i)+1;
					cont = 1;
				}
			}
			else{
				if(right_child->priority < last_elem->priority){
					dynarray_set(pq->heap, i, right_child);
					dynarray_set(pq->heap, (2*i)+2, last_elem);	
					i = (2*i)+2;
					cont = 1;
				}
			}
		}
		else if(dynarray_size(pq->heap) > (2*i)+1){
			struct pq_elem* left_child = dynarray_get(pq->heap, (2*i)+1);	
			if(left_child->priority < last_elem->priority){
				dynarray_set(pq->heap, i, left_child);
				dynarray_set(pq->heap, (2*i)+1, last_elem);
